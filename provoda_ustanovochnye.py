...
url = 'https://bystrokabel.ru/item/provoda_ustanovochnye'
...
# with open('provoda_ustanovochnye.html', 'w', encoding='UTF-8') as file:
#     file.write(src)
...
# with open('provoda_ustanovochnye.html', 'r', encoding='UTF-8') as file:
#     src = file.read()
# print(src)
...
# with open('provoda_ustanovochnye_urls.json', 'w', encoding='UTF-8') as file:
...
with open('provoda_ustanovochnye_urls.json', encoding='UTF-8') as file:
    all_cable_types = json.load(file)
# print(len(all_cable_types))
...
# count = 0
# all_files_cable_types = os.listdir('data_provoda_ustanovochnye')
# for cable_name, cable_link in all_cable_types.items():
#     # if count == 0:
#     if ' ' in cable_name:
#         cable_name = cable_name.replace(' ', '_')
#
#     # если сработает капча, то проверка ниже нужно, чтобы повторно запустить цикл
#     if cable_name + '.html' in all_files_cable_types:
#         continue
#     else:
#         sleep(random.randrange(1, 4))
#         print(f'нет такого - {cable_name}')
#
#         req = requests.get(url=cable_link, headers=headers)
#         src = req.text
#
#     with open(f'data_provoda_ustanovochnye\\{cable_name}.html', 'w', encoding='UTF-8') as file:
#         file.write(src)
...
#     file_name = 'data_provoda_ustanovochnye\\' + i
...
# with open('all_diameters_provoda_ustanovochnye\\all_diameters.json', 'w', encoding='UTF-8') as file:
...
with open('all_diameters_provoda_ustanovochnye\\all_diameters.json', encoding='UTF-8') as file:
...
# count = 0
# folder_list = os.listdir('all_diameters_provoda_ustanovochnye')
# print(f'Всего итераций {len(all_cable_diameters) - len(folder_list)}')
# for cable_name_diameter, cable_link in all_cable_diameters.items():
#     #     # если сработает капча, то проверка ниже нужно, чтобы повторно запустить цикл
#     if cable_name_diameter + '.html' in folder_list:
#         continue
#     else:
#         sleep(random.randrange(1, 3))
#         # print(f'нет такого - {cable_name_diameter}')
#
#         req = requests.get(url=cable_link, headers=headers)
...
#             with open(f'all_diameters_provoda_ustanovochnye\\{file_name[7:]}.html', 'w', encoding='UTF-8') as file:
...
all_files_cable_diameters = os.listdir('all_diameters_provoda_ustanovochnye')
...
for i in all_files_cable_diameters:
    file_name = 'all_diameters_provoda_ustanovochnye\\' + i
    with open(file_name, 'r', encoding='UTF-8') as file:
        src = file.read()
    soup = BeautifulSoup(src, 'html.parser')
    ...
    # Определение ссылки провода из файла all_diameters.json:
    for cable_name_diameter, cable_link in all_cable_diameters.items():
...
data.to_csv('data\\provoda_ustanovochnye.csv', index=False)
